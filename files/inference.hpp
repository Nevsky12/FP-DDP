...
public:
    ...
    [[nodiscard]] std::expected<Solution<R>, Failure<R>> run() {
        sol_.t0_ = iv_.t0;
        t_       = iv_.t0;
...
private:
    ...
    [[nodiscard]] std::expected<Solution<R>, Failure<R>> finish() {
        sol_.t1_    = t_;
        sol_.y_     = y_;
        sol_.stats_ = st_;
    ...
    [[nodiscard]] std::expected<Solution<R>, Failure<R>> fail(Breakdown kind) {
        sol_.t1_    = t_;
        sol_.y_     = y_;
        sol_.stats_ = st_;
        return std::unexpected(Failure<R>{kind, t_, std::move(sol_)});
    }
...
[[nodiscard]] std::expected<Solution<R>, Failure<R>>
