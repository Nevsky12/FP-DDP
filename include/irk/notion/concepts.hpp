...
// A controlled field additionally exposes ∂f/∂u (the control Jacobian, sized
// n×m, fully overwritten) and its control dimension m — the data the discrete
// forward sensitivities A = ∂x⁺/∂x, B = ∂x⁺/∂u require.
template <typename F, typename R>
concept ControlledField =
    ProvidesJacobian<F, R> &&
    requires(const F f, R t, std::span<const R> y, essence::Matrix<R>& B) {
        { f.control_jacobian(t, y, B) } -> std::same_as<void>;
        { f.control_dim() } -> std::convertible_to<std::size_t>;
    };
